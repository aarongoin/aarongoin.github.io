...
class Fairy extends React.Component {
	...
	render() {
		...
		return (
			...
			>
				<span style={Object.assign({}, Style.bubble, this.state.bubblePos, { color: this.props.color })}>
					{this.state.text}
				</span>
                <div style={Object.assign({}, Style.body, propStyle)}></div>
				<div
				...
				>
					<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 1024 1024">
                        <path style={{ fill: "rgba(235,255,255,0.95)" }} d={fairyWingPath}/>
					</svg>
				</div>
				<div
					style={Object.assign({}, Style.wing, Style.lowerRightWing, { transform: `rotate(${(this.state.wingAngle)}deg)`})}
				>
					<svg style={{ transform: 'scale(1, -1)' }} xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 1024 1024">
                        <path style={{ fill: "rgba(235,255,255,0.95)" }} d={fairyWingPath}/>
					</svg>
				</div>
				<div
				...
				>
					<svg style={{ transform: 'scale(-1, 1)' }} xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 1024 1024">
                        <path style={{ fill: "rgba(235,255,255,0.95)" }} d={fairyWingPath}/>
					</svg>
				</div>
				<div
	...
	move(top, left) {
		...
		top = top ? clamp(top, this.margin, this.height) : rand(this.height - this.state.style.width, this.margin)
		left = left ? clamp(left, this.margin, this.width) : rand(this.width - this.state.style.width, this.margin)
